...
const getAll = async(req, res) => {
    ...
    try {
        count = await Courses.find(filter).count();
        const result = await Courses.find(filter)
            .skip(query.skip)
            .limit(query.limit)
            .select('-__v');
...
const getById = async(req, res) => {
    ...
    try{
        const result = await Courses.findOne({_id: mongoose.Types.ObjectId(id)}).select('-__v')
...
const updateStatus = async(req, res) => {
    ...
    try{
        ...
        if(reqPayload.status == 100){
            result = await Courses.deleteMany(filter);
            message = "Selected courses are permanently deleted successfully!";
        }else{
            result = await Courses.updateMany(filter, {status: reqPayload.status});
            message = "Selected courses status updated successfully!";
        }
